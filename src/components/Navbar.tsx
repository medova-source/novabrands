...
const capabilities = [
  ...
  { to: "/manufacturing/powders", label: "Powders", desc: "Sticks, jars & bulk." },
  { to: "/manufacturing/liquids", label: "Liquids", desc: "Tinctures, shots & syrups." },
];

const NavDropdown = ({ label, items, isDarkNav }: { label: string; items: { to: string; label: string; desc: string }[]; isDarkNav: boolean }) => (
  <div className="group relative">
    <button className={`flex items-center gap-1.5 py-2 text-[13px] font-medium transition-colors ${!isDarkNav ? "text-foreground/80 hover:text-foreground" : "text-white/80 hover:text-white"}`}>
      {label}
  ...
  return (
    <header
      className={`fixed inset-x-0 top-0 z-50 transition-all duration-500 ${
        scrolled ? "bg-background/85 backdrop-blur-xl border-b border-[hsl(var(--rule))]" : "bg-transparent"
      }`}
    >
      <div className="container-x flex h-16 items-center justify-between md:h-20">
        ...
        <nav className="hidden items-center gap-8 lg:flex">
          <NavDropdown label="Custom Products" items={customProducts} isDarkNav={isDarkNav} />
          <NavDropdown label="Manufacturing" items={capabilities} isDarkNav={isDarkNav} />
          <NavLink to="/distribution" className={({ isActive }) => `text-[13px] font-medium transition-colors ${isActive ? (!isDarkNav ? "text-foreground" : "text-white") : (!isDarkNav ? "text-foreground/80 hover:text-foreground" : "text-white/80 hover:text-white")}`}>
            3PL Distribution
          </NavLink>
          <NavLink to="/about" className={({ isActive }) => `text-[13px] font-medium transition-colors ${isActive ? (!isDarkNav ? "text-foreground" : "text-white") : (!isDarkNav ? "text-foreground/80 hover:text-foreground" : "text-white/80 hover:text-white")}`}>
            About
          </NavLink>
          <NavLink to="/how-we-work" className={({ isActive }) => `text-[13px] font-medium transition-colors ${isActive ? (!isDarkNav ? "text-foreground" : "text-white") : (!isDarkNav ? "text-foreground/80 hover:text-foreground" : "text-white/80 hover:text-white")}`}>
            How We Work
          </NavLink>
        </nav>
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={() => setOpen(true)} className={`grid h-10 w-10 place-items-center rounded-full border lg:hidden ${!isDarkNav ? "border-[hsl(var(--rule))] text-foreground" : "border-white/20 text-white"}`} aria-label="Open menu">
      ...
      {/* Mobile menu */}
      <div className={`fixed inset-0 z-50 bg-background transition-opacity duration-500 lg:hidden ${open ? "pointer-events-auto opacity-100" : "pointer-events-none opacity-0"}`}>
        <div className="flex h-16 items-center justify-between px-6 md:h-20 md:px-10">
          <Logo />
          <button onClick={() => setOpen(false)} className="grid h-10 w-10 place-items-center rounded-full border border-[hsl(var(--rule))]" aria-label="Close menu">
            <X className="h-4 w-4" />
          </button>
        </div>
        <div className="container-x grid gap-1 py-6">
          ...
          <Link to="/contact" className="mt-6 btn-primary w-fit">Start Your Project <ArrowUpRight className="h-4 w-4" /></Link>
        </div>
      </div>
    </header>
  );
